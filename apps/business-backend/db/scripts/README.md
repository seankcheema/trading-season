...
4. `0004-import-synthetic-market-data.py --database-url URL [--replace]` validates again and imports with bounded PostgreSQL COPY batches in one transaction. An identical completed import is skipped; changed or candle-only session data requires `--replace`.
