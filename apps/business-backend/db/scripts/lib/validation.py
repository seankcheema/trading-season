...
from .common import SESSION_SECONDS, archive_fingerprint, sha256
...
def validate_archive(dataset: Path, manifest: dict[str, Any] | None = None) -> dict[str, int]:
    ...
    with duckdb.connect(config={"threads": 2}) as db:
        tick_count, bad_ticks, unique_sequences, unique_points = db.execute("""
            SELECT count(*), count(*) FILTER (WHERE price <= 0 OR bid <= 0 OR ask <= bid
              ...
              OR t < session_start OR t >= session_start + ? OR (t-session_start) % 1 <> 0),
              count(DISTINCT sequence_number), count(DISTINCT (symbol,t)) FROM read_parquet(?)
        """, [SESSION_SECONDS, ticks]).fetchone()
        sequence_errors = db.execute("""SELECT count(*) FROM (SELECT sequence_number,
          row_number() OVER (ORDER BY t,symbol) expected FROM read_parquet(?)) WHERE sequence_number<>expected""",[ticks]).fetchone()[0]
    ...
    if (tick_count, candle_count) != (expected_ticks, expected_candles) or bad_ticks or bad_candles or unique_sequences != tick_count or unique_points != tick_count or sequence_errors or coverage_errors or disagreement:
        raise ValueError(f"Archive validation failed: {tick_count=}, {candle_count=}, {bad_ticks=}, {bad_candles=}, {unique_sequences=}, {unique_points=}, {sequence_errors=}, {coverage_errors=}, {disagreement=}")
