...
| POST /api/me/accounts | Bearer token; name, currency, optional initialDeposit | Created cash account |
| GET /api/me/portfolios | Bearer token | Portfolios in the current user's accounts |
| POST /api/me/portfolios | Bearer token; accountId of an owned account, name, optional description | Created portfolio |
| PUT /api/me/portfolios/{portfolioId} | Bearer token; owned portfolio id; name, optional description | Updated portfolio |
...
| GET /api/accounts/{accountId}/cash-transactions | Bearer token; owned account id; optional limit | Account cash transactions, newest first |
...
- The dashboard reads and changes the signed-in user's accounts, portfolios and cash through the planned account endpoints above, which the Java backend does not serve yet; see [AccountStore](../../apps/business-logic-ui/src/app/dashboard/accounts/account-store.service.ts) and [the models it expects](../../apps/business-logic-ui/src/app/dashboard/accounts/account.models.ts). An account is returned as accountId, name, currency, cashBalance and openedDate; a portfolio as portfolioId, accountId, name, description and createdAt; a cash transaction as cashTransactionId, accountId, a positive amount, reason and createdAt. After every successful change the UI reloads the affected lists rather than trusting the response body. It maps 400 to the backend's error string, 403 and 404 to an unavailable account or portfolio, and 409 or 422 to a duplicate name or, for withdrawals, insufficient funds. The UI lists only what these endpoints return for the caller, drops portfolios whose account is not among them, and sends no change for an account or portfolio id outside that set; the backend must still enforce ownership from the token's sub.
