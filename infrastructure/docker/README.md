...
[Dockerfile.playwright](Dockerfile.playwright) is the slim end-to-end runner the pipeline builds for each `@playwright/test` version. It contains only `chromium-headless-shell`, ffmpeg, and their system libraries; the pipeline mounts the workspace and the Jenkins Node toolchain into it at run time.
