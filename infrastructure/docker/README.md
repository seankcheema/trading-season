...
The image installs Node 24.x, which is compatible with this repository's Angular 21.2.x requirement. The active pipeline expects a configured native Jenkins agent with the project-standard Node 24.8.0 toolchain; building this image does not change that agent's tools. Review [operations](../../docs/guides/operations.md#ci-and-artifacts) before using the example.
